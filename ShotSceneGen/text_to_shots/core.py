...
class TextToShotsProcessor:
    ...
    def __init__(self, api_key: str = None, references_folder: str = None):
        """
        Initialize the processor.

        Args:
            api_key: OpenAI API key. If None, will try to load from environment
            references_folder: Path to folder containing reference documents. If None, uses package's outputs folder
        """
        ...
        # Set references folder - use package's outputs folder as default
        if references_folder is None:
            package_dir = os.path.dirname(os.path.abspath(__file__))
            self.references_folder = os.path.join(package_dir, "outputs")
        else:
